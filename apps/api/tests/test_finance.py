...
def test_finance_flow(client, db, auth_headers):
    ...
    resp = client.get(f"/api/clubs/{club_id}/finance/state", headers=auth_headers)
    assert resp.status_code == 200
    # Balance = 0 + 50,000,000 (Distribution in Aug) + 1,000 (Base Sponsor) - 200 (Base Cost) - 2,800,000 (Default Staff Cost for 7 roles @ 4.8M/yr)
    assert resp.json()["balance"] == 47200800.0
    ...
    assert snapshots[0]["closing_balance"] == 47200800.0
    assert snapshots[0]["income_total"] == 50001000.0
    assert snapshots[0]["expense_total"] == -2800200.0
    
    # 5. Idempotency Check
    # Call apply_finance_for_turn again manually
    finance.apply_finance_for_turn(db, UUID(season_id), UUID(turn_id))
    
    # Balance should still be 47,200,800.0
    state = finance.get_financial_state(db, UUID(club_id))
    assert state.balance == 47200800.0
