...
def test_finance_flow(client, db, auth_headers):
    ...
    resp = client.get(f"/api/clubs/{club_id}/finance/state", headers=auth_headers)
    assert resp.status_code == 200
    # Balance = opening cash + 20M distribution + 50M annual sponsors + 1,000 base sponsor
    # - 2.5M reinforcement - 250k team operation - 2.8M staff - 200 admin override.
    assert resp.json()["balance"] == 164450800.0
    ...
    assert snapshots[0]["closing_balance"] == 164450800.0
    assert snapshots[0]["income_total"] == 70001000.0
    ...
    assert state.balance == 164450800.0
