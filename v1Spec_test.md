...
- $Pop=1,000,000$
- $Cap=20,000$
- 初期ファンベース率 $f(0)=0.01 \to FB(0)=10,000$
- 上限 $f_{max}=0.25$
...
今期順位スコアはリーグ中央を0.5、1順位差を0.225として計算する。

$$ Perf_i(t)=clip\left(0.5+0.225\left(\frac{N+1}{2}-rank_i(t)\right),0,1\right) $$

- 直前順位がない開幕月、順位表にクラブがない場合、クラブ数が1以下の場合は0.5
- 2〜5クラブでは1順位差のロジット効果を $0.8\times0.225=0.18$ に固定
- 同じスコアをファンベース成長式の $Perf$ にも使用
