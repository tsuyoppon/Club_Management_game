...
**現在の実装状況**: 約90%完了（PR1-PR10完了、ドキュメント・パッケージング残）
...
- [x] `show sponsor_status` - スポンサー状態
  - `--pipeline` オプション（4-6月）
  - `--next` オプション（7月）
- [x] `help` / `help <command>` - ヘルプ表示

#### 10.3 入力コマンド
- [x] `input` - 月次入力
  - `--sales-expense`, `--promo-expense`, `--hometown-expense`
  - `--next-home-promo`, `--additional-reinforcement`, `--rho-new`
...
### PR10完了基準
- [x] 全参照コマンドが実装されている
- [x] 全入力コマンドが実装されている（input/commit/view）
- [x] CLIが安定して動作
- [x] エラーハンドリングが整備されている
- [ ] ドキュメントが整備されている
- [x] テストカバレッジ70%以上
...
2. **パッケージング**
   - pip install対応（pyproject.toml/setup.py）
   - 配布手順の整備
...
**最終更新日**: 2025年12月
**現在の実装状況**: PR1-PR10完了（約90%）、ドキュメント・パッケージング残
