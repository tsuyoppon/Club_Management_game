...
Web Multiplayer デモをすぐ開始できる状態まで作る場合は、リポジトリ直下で以下を実行してください。

```bash
./scripts/demo_env.py up --reset
```

このコマンドは Docker 起動、DB マイグレーション、Web ルーム/ゲーム作成、クラブ claim/ready、開始処理、再実演用の状態保存まで行います。

詳細な運用、reset、archive、resume の扱いは [`docs/DEMO_ENV_RUNBOOK.md`](DEMO_ENV_RUNBOOK.md) を参照してください。

---

## 目次

0. [当日用ワンコマンド起動](#0-当日用ワンコマンド起動)
1. [前提条件](#1-前提条件)
2. [環境構築・起動](#2-環境構築起動)
